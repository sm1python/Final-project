...
async function registerUser(username, password, conPass, email) {
    // to save user details to persistence layer
    if (password != conPass) {
        return false
    }
    await persistence.saveUser(username, password, email)
}
...
async function checkKey(data) {
    let t = await persistence.getKey(data.key)
    if (!t) {
        return false
    }
    await persistence.resetPassword(data)
    return true
}

module.exports = {
    checkLogin, startSession, getSessionData, deleteSession, registerUser, getLocations, savePost, updatePass, resetPass, checkKey
}
